##Melanoma 1.1 dataset
creating signature matrix and simulated samples
...
saveRDS(training_exp,"sigmatrix_mel_1.1.rds")
...
```{r}
set.seed(999)
bulk_mel_1.1 = make_bulk_eset(eset=testing_exp,
                           cell_fractions = cell_fractions,
                           n=500)

fData(bulk_mel_1.1) = rownames_to_column(fData(bulk_mel_1.1),var = "gene_symbol")

saveRDS(bulk_mel_1.1,"pseudobulk_data/bulk_melanoma_1.1.rds")



bulk_mel_1.1_txt = as.data.frame(exprs(bulk_mel_1.1))%>%
  rownames_to_column("Gene")



write.table(bulk_mel_1.1_txt, sep = '\t', "pseudobulk_data/bulk_mel_1.1.txt", row.names = FALSE, quote = FALSE, col.names = TRUE)
...
saveRDS(finalsigmatrix, "sigmatrix_mel_1.1.rds")
write.table(finalsigmatrix, sep = '\t', "sigmatrix_mel_1.1.txt", row.names = TRUE, quote = FALSE)


```

sigmatrix without cancer cells
```{r}
data2_nocan <- subset(data2, Cell_type !="Malignant")

sigmatrix_nocan <- as.data.frame(t(data2_nocan))

finalsigmatrix_nocan <- sigmatrix_nocan%>%
  unname()

row.names(finalsigmatrix_nocan)[1] <- " "

write.table(finalsigmatrix_nocan, sep = '\t', "sigmatrix_mel_1.1_nocan.txt", row.names = TRUE, quote = FALSE)
